...
// using ATC_RADAR_CHANNEL macro from Msg_structs.h
...
Aircraft::Aircraft(int id, double x, double y, double z, double sx, double sy, double sz, int t)
	...
	int result = pthread_create(&thread_id, NULL, updatePositionThread, (void*)this);
	if (result != 0) {
		std::cerr << "Aircraft " << id << ": Failed to create thread, error code: " << result << std::endl;
	}
}
...
int Aircraft::updatePosition() {
    ...
    if ((Radar_id = name_open(ATC_RADAR_CHANNEL, 0)) == -1) {
    ...
    std::string myChannelName = getAircraftChannel(id);  // Uses my inline function from Msg_structs.h
    name_attach_t* Plane_channel = name_attach(NULL, myChannelName.c_str(), 0); // For server
    ...
    while (true) {
        ...
        if (rcvid != -1) {
            ...
            if (isInterProcess){  //sporadic
                ...
                switch (receivedMsg->type) {
                    case MessageType::REQUEST_CHANGE_OF_HEADING: {
                        ...
                        if (receivedMsg->dataSize >= sizeof(msg_change_heading)) {
                            ...
                            speedX = headingData->VelocityX;
                            speedY = headingData->VelocityY;
                            speedZ = headingData->VelocityZ;
                            if (headingData->altitude > 0) {
                                posZ = headingData->altitude;
                            }
                            std::cout << "Aircraft " << id << ": Heading changed to (" << speedX << ", " << speedY << ", " << speedZ << ")" << std::endl;
                        }
                        break;
                    }
                    case MessageType::REQUEST_CHANGE_POSITION: {
                        if (receivedMsg->dataSize >= sizeof(msg_change_position)) {
                            ...
                            posY = posData->y;
                            posZ = posData->z;
                            std::cout << "Aircraft " << id << ": Position changed to (" << posX << ", " << posY << ", " << posZ << ")" << std::endl;
                        }
                        break;
                    }
                    case MessageType::REQUEST_CHANGE_ALTITUDE: {
                        if (receivedMsg->dataSize >= sizeof(double)) {
                            double newAlt;
                            std::memcpy(&newAlt, receivedMsg->data.data(), sizeof(double));
                            posZ = newAlt;
                            std::cout << "Aircraft " << id << ": Altitude changed to " << posZ << std::endl;
                        }
                        break;
                    }
                    default:
                ...
                MsgReply(rcvid, 0, &replyStatus, sizeof(replyStatus));
            } else {  //periodic
...
Message Aircraft::createEnterAirspaceMessage(int planeID){
	Message msg;
	msg.header = false;  // intra-process message
...
Message Aircraft::createPositionUpdateMessage(int planeID, const msg_plane_info& info) {
    ...
    msg.dataSize = sizeof(msg_plane_info);
